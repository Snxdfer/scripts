...
if outdated then
    if not getDataFileValue("disableVersionAlert") then -- true if user has presses "Don't show again" before
        notify(
...
internetSlang:Add("What does 'IIRC' stand for?", {"If I Recall Correctly (If I Remember Correctly)", "It Is Really Cool", "I'm Incredibly Rich, Child", "Interesting Information Requires Consideration"}, 2)
...
local function UpdateCategoryTable(categoryName: string, difficulties: table)
    for difficulty, quizzes in difficulties do
        for index, quiz in quizzes do
            ...
            if #quizzes > 1 and categories.numberOfDifficulties[categoryName] > 1 then
                listName = categoryName.."-"..difficulty..index -- ex: flags-easy1, flags-easy2, flags-medium
                table.insert(categoryTable, listName)
            elseif categories.numberOfDifficulties[categoryName] > 1 and difficulty ~= "" then -- if multiple difficulties exist but only one quiz in current difficulty, only add the difficulty at the end
            ...
            elseif #quizzes > 1 or difficulty == "" then
...
table.sort(categoriesToSendDetailed)
...
local function sendCategories()
    ...
    elseif quizRunning then
        notify("Can't send cateogry list", "Can't send the category list while quiz is running. Stop the current quiz or wait for it to end")
    end
end
...
local function choseAutoplayCategory()
    ...
    for _, difficulty in autoPlayDifficulties do -- do this each time in case the user changes the autoplay filter
        if autoplayChosenCategories[difficulty] and not #autoplayChosenCategories[difficulty] == numberOfCategoriesPerDifficulty[difficulty] then
            ...
            break
        elseif not autoplayChosenCategories[difficulty] then -- if the difficulty has not been chosen yet at all
            autoplayComplete = false
            break
        end
    end
...
local leaderboardTab = window:CreateTab("Leaderborad", 97885193604839)
...
autoplayFilterDropdown = settingsTab:CreateDropdown({
